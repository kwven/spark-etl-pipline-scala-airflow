...
class ForbesAnalyzer(sparkSession: SparkSession) {
  ...
  def top10SelfMade(df: Dataset[Row]): Dataset[Row] = {
    df.filter(col(ForbesAnalyzer.SELF_MADE) === "True")
      .select(col(ForbesAnalyzer.RANK), col(ForbesAnalyzer.PERSON_NAME), col(ForbesAnalyzer.AGE), col(ForbesAnalyzer.FINAL_WORTH))
      .sort(col(ForbesAnalyzer.FINAL_WORTH).desc)
      .limit(10)
  }
}
