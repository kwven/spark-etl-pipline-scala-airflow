...
class ForbesAnalyzerTest extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("countCountriesForbes should count richest guys by country") {
    ...
    assert(countryCountsDF.filter(col(ForbesAnalyzer.COUNTRY) === "USA").head().getAs[Long](ForbesAnalyzer.COUNTRY_NO) === 3L)
    assert(countryCountsDF.filter(col(ForbesAnalyzer.COUNTRY) === "India").head().getAs[Long](ForbesAnalyzer.COUNTRY_NO) === 1L)
