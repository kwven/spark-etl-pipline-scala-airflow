...
compose_for_project exec -T namenode /bin/bash -c "\
  ...
  $HDFS_BIN dfs -rm -r -f $HDFS_RAW_DIR || true && \
  $HDFS_BIN dfs -rm -r -f $HDFS_PARQUET_RESULTS_DIR || true && \
  $HDFS_BIN dfs -rm -r -f $HDFS_CSV_RESULTS_DIR || true && \
  $HDFS_BIN dfs -mkdir -p $HDFS_RAW_DIR"
